...
# Pseudocode
  ...
  * Identify a question
  * Wrangle the data
  * ID some candidate models
  * Select among candidate models using AIC/BIC
  * Select among candidate models using K-fold cross validation
  * Select among candidate models using areas under Receiver Operating Characteristic Curve
...
# == means you want it to match the entire vector in the order you list them in
# %in% does Adelie OR Chinstrap no matter the order
# to be safe you can use the == every time you use filter
  mutate(species = fct_drop(species)) %>% 
  select(-year) %>% 
    drop_na()

```
## Lets check out trends across variables
```{r}
